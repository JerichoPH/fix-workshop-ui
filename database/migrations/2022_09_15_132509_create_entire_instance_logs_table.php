...
class CreateEntireInstanceLogsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('entire_instance_logs', function (Blueprint $table) {
            ...
            $table->string('organization_paragraph_uuid',36)->nullable(false)->default('')->comment('所属站段UUID');
            $table->index('organization_paragraph_uuid');
            $table->string('organization_workshop_uuid',36)->nullable(false)->default('')->comment('所属车间UUID');
            ...
            $table->string('location_line_uuid',36)->nullable(false)->default('')->comment('所属线别UUID');
            $table->index('location_line_uuid');
            $table->string('location_station_uuid',36)->nullable(false)->default('')->comment('所属线别UUID');
            $table->index('location_station_uuid');
            $table->string('location_section_uuid',36)->nullable(false)->default('')->comment('所属区间UUID');
