...
class CreateOrganizationParagraphsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('organization_paragraphs', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->char("unique_code", 4)->unique()->nullable(false)->comment("站段代码");
            $table->string("name", 64)->unique()->nullable(false)->comment("站段名称");
            ...
            $table->foreign("organization_railway_unique_code")->references("unique_code")->on("organization_railways")->comment("所属路局");
        });
    }
