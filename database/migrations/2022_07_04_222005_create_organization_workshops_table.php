...
class CreateOrganizationWorkshopsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('organization_workshops', function (Blueprint $table) {
            ...
            $table->foreign("organization_workshop_type_unique_code","oWorkshops__owtuc")->references("unique_code")->on("organization_workshop_types")->onUpdate("cascade")->comment("所属车间类型");
            $table->char("organization_paragraph_unique_code", 4)->nullable(false)->comment("所属站段代码");
            $table->foreign("organization_paragraph_unique_code","oWorkshops__opuc")->references("unique_code")->on("organization_paragraphs")->onUpdate("cascade")->comment("所属站段");
        });
    }
