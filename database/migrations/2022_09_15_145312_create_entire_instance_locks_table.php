...
class CreateEntireInstanceLocksTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('entire_instance_locks', function (Blueprint $table) {
            ...
            $table->string('business_order_table_name', 128)->nullable(false)->default('')->comment('业务相关单据表名称');
            $table->string('business_order_uuid', 36)->nullable(false)->default('')->comment('业务相关表UUID');
            $table->string('business_item_table_name', 128)->nullable(false)->default('')->comment('业务相关子项表名称');
            $table->string('business_item_uuid', 36)->nullable(false)->default('')->comment('业务相关子项UUID');
        });
    }
