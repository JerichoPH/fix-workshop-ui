...
class CreateEntireInstanceLogsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('entire_instance_logs', function (Blueprint $table) {
            ...
            $table->text('url')->nullable(true)->comment('网址');
            ...
            $table->string('organization_paragraph_uuid',36)->nullable(true)->comment('所属站段UUID');
            $table->index('organization_paragraph_uuid');
            $table->string('organization_workshop_uuid',36)->nullable(true)->comment('所属车间UUID');
            ...
            $table->string('location_station_uuid',36)->nullable(true)->comment('所属线别UUID');
            $table->index('location_station_uuid');
            $table->string('location_section_uuid',36)->nullable(true)->comment('所属区间UUID');
            $table->index('location_section_uuid');
            $table->string('location_center_uuid',36)->nullable(true)->comment('所属中心UUID');
            $table->index('location_center_uuid');
            $table->string('location_railroad_grade_cross_uuid',36)->nullable(true)->comment('所属道口UUID');
