...
class CreateEntireInstancesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('entire_instances', function (Blueprint $table) {
            ...
            $table->string('serial_number', 64)->nullable(false)->default('')->comment('所编号');
            ...
            $table->string('factory_made_serial_number',64)->nullable(false)->default('')->comment('出厂编号');
            $table->date('factory_made_at')->nullable(false)->comment('出厂日期');
            ...
            $table->string('use_repair_current_spot_checker_name',64)->nullable(false)->default('')->comment('抽验人');
            $table->datetime('use_repair_last_fixed_at')->comment('上次检修时间');
            $table->string('use_repair_last_fixer_name',64)->nullable(false)->default('')->comment('上次检修人');
            $table->datetime('use_repair_last_checked_at')->comment('上次验收时间');
            $table->string('use_repair_last_checker_name',64)->nullable(false)->default('')->comment('上次验收人');
            $table->datetime('use_repair_last_spot_checked_at')->comment('上次抽验时间');
            $table->string('use_repair_last_spot_checker_name',64)->nullable(false)->default('')->comment('上次抽验人');
            $table->string('belong_to_organization_railway_uuid',64)->nullable(false)->default('')->comment('所属路局UUID');
            $table->index('belong_to_organization_railway_uuid');
            $table->string('belong_to_organization_paragraph_uuid',64)->nullable(false)->default('')->comment('所属站段UUID');
            $table->index('belong_to_organization_paragraph_uuid');
            $table->string('belong_to_organization_workshop_uuid',64)->nullable(false)->default('')->comment('所属车间UUID');
            $table->index('belong_to_organization_workshop_uuid');
            $table->string('belong_to_organization_work_area_uuid',64)->nullable(false)->default('')->comment('所属工区UUID');
            $table->index('belong_to_organization_work_area_uuid');
            $table->string('use_place_current_organization_workshop_uuid',64)->nullable(false)->default('')->comment('上道地点：车间UUID');
            $table->index('use_place_current_organization_workshop_uuid','entire_instances__upcowu');
            $table->string('use_place_current_organization_work_area_uuid',64)->nullable(false)->default('')->comment('上道地点：工区UUID');
            $table->index('use_place_current_organization_work_area_uuid','entire_instances__upcowau');
            $table->string('use_place_current_location_line_uuid',64)->nullable(false)->default('')->comment('上道地点：线别UUID');
            $table->index('use_place_current_location_line_uuid','entire_instances__upcllu');
            $table->string('use_place_current_location_station_uuid',64)->nullable(false)->default('')->comment('上道地点：站场UUID');
            $table->index('use_place_current_location_station_uuid','entire_instances__upclsu');
            $table->string('use_place_current_location_section_uuid',64)->nullable(false)->default('')->comment('上道地点：工区UUID');
            ...
            $table->string('use_place_last_location_line_uuid',64)->nullable(false)->default('')->comment('上次上道地点：线别UUID');
            $table->index('use_place_last_location_line_uuid','entire_instances__uplllu');
            $table->string('use_place_last_location_station_uuid',64)->nullable(false)->default('')->comment('上次上道地点：站场UUID');
            $table->index('use_place_last_location_station_uuid','entire_instances__upllsu');
            $table->string('use_place_last_location_section_uuid',64)->nullable(false)->default('')->comment('上次上道地点：工区UUID');
