...
class CreatePositionIndoorTiersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('position_indoor_tiers', function (Blueprint $table) {
            ...
            $table->string('unique_code', 13)->nullable(false)->comment('室内上道位置柜架层代码（13位）');
            $table->index('unique_code');
            $table->string('name', 64)->nullable(false)->comment('室内上道位置柜架层名称');
            $table->string('position_indoor_cabinet_uuid', 36)->nullable(false)->comment('所属室内上道位置柜架UUID');
            $table->index('position_indoor_cabinet_uuid');
        });
    }
