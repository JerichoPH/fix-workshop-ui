...
class CreatePivotOrganizationLineAndOrganizationStationsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('pivot_organization_line_and_organization_stations', function (Blueprint $table) {
            ...
            $table->foreign("organization_line_id","fPOLAOS__oli")->references("id")->on("organization_lines")->onUpdate("cascade")->comment("所属线别");
            $table->unsignedBigInteger("organization_station_id")->nullable(false)->comment("所属站场ID");
            $table->foreign("organization_station_id","fPOLAOS__osi")->references("id")->on("organization_stations")->onUpdate("cascade")->comment("所属站场");
        });
    }
