                ...
                <form class="form-horizontal" id="frmUpdate">
                    <div class="box box-solid">
                        ...
                        <div class="box-footer">
                            <a href="{{ route('web.LocationCenter:index') }}" class="btn btn-default pull-left btn-sm"><i class="fa fa-arrow-left">&nbsp;</i>返回</a>
...
@section('script')
    <script>
        ...
        function fnInit() {
            $.ajax({
                url: `{{ route("web.LocationCenter:Show", ["uuid" => $uuid,]) }}`,
                type: 'get',
                data: {},
                async: true,
                beforeSend: function () {
                ...
                success: function (res) {
                    console.log(`{{ route("web.LocationCenter:Show", ["uuid" => $uuid,]) }} success:`, res);

                    locationCenter = res["content"]["location_center"];

                    let {unique_code: uniqueCode, name, be_enable: beEnable, location_lines: locationLines,} = locationCenter;
                    ...
                    fnFillSelOrganizationWorkshop(locationCenter["organization_workshop"]["uuid"]);
                    fnFillSelOrganizationWorkArea(locationCenter["organization_workshop"]["uuid"], locationCenter["organization_work_area"]["uuid"]);
                    // 已经绑定的线别
                ...
                error: function (err) {
                    console.log(`{{ route("web.LocationCenter:Show", ["uuid" => $uuid,]) }} fail:`, err);
        ...
        function fnUpdate() {
            ...
            $.ajax({
                url: `{{ route('web.LocationCenter:Update', ["uuid" => $uuid , ]) }}`,
                type: 'put',
                data,
                success: function (res) {
                    console.log(`{{ route('web.LocationCenter:Update', ["uuid" => $uuid, ]) }} success:`, res);
                    layer.close(loading);
                    layer.msg(res.msg, {time: 1000,});
                },
                error: function (err) {
                    console.log(`{{ route('web.LocationCenter:Update', ["uuid" => $uuid, ]) }} fail:`, err);
                    layer.close(loading);
                    layer.msg(err["responseJSON"]["msg"], {icon: 2,}, function () {
                        if (err.status === 401) location.href = '{{ route('web.Authorization:getLogin') }}';
                    });
        ...
        function fnBindLocationLines() {
            ...
            $.ajax({
                url: `{{ route("web.LocationCenter:putBindLocationLines", ["uuid" => $uuid,]) }}`,
                type: 'put',
                data,
                async: true,
                success: res => {
                ...
                error: err => {
                    console.log(`{{ route("web.LocationCenter:putBindLocationLines", ["uuid" => $uuid,]) }} fail:`, err);
                    layer.close(loading);
                    layer.msg(err["responseJSON"]["msg"], {icon: 2,}, function () {
                        if (err.status === 401) location.href = '{{ route('web.Authorization:getLogin') }}';
                    });
