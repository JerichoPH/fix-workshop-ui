...
class CreateKindEntireTypesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('kind_entire_types', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->char("unique_code", 5)->unique("uiKET__uniqueCode")->nullable(false)->comment("种类代码");
            $table->string("name", 64)->unique("uiKET__name")->nullable(false)->comment("种类代码");
            ...
            $table->foreign("kind_category_unique_code","uiKET__kindCategoryUniqueCode")->references("unique_code")->on("kind_categories")->onUpdate("cascade")->comment("所属种类");
        });
    }
