...
//		$res = preg_match('/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i', $val);
    ...
    {
        $val = (string)$val;
        $value_len = TextHelper::strLen($val);
        if ($value_len != 11) {
            return false;
        }
     ...
     * @return bool
     */
    public static function firstErrorByRequest(Request $data, $request, $response = null, $errorCode = 420)
    {
     ...
     * @return bool
     */
    public static function entireErrorsByRequest(Request $data, $request, $response = null, $errorCode = 420)
    {
        $v = Validator::make($data->all(), $request->rules(), $request->messages());
