...
class CreateEntireInstancesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('entire_instances', function (Blueprint $table) {
            ...
            $table->string('serial_number', 64)->nullable(false)->comment('所编号');
            ...
            $table->string('factory_made_serial_number',64)->comment('出厂编号');
            $table->date('factory_made_at')->comment('出厂日期');
            ...
            $table->datetime('use_repair_current_spot_checked_at')->comment('抽验时间');
            $table->string('use_repair_current_spot_checker_name',64)->comment('抽验人');
            $table->datetime('use_repair_last_fixed_at')->comment('上次检修时间');
            $table->string('use_repair_last_fixer_name',64)->comment('上次检修人');
            $table->datetime('use_repair_last_checked_at')->comment('上次验收时间');
            $table->string('use_repair_last_checker_name',64)->comment('上次验收人');
            $table->datetime('use_repair_last_spot_checked_at')->comment('上次抽验时间');
            $table->string('use_repair_last_spot_checker_name',64)->comment('上次抽验人');
            $table->string('belong_to_organization_railway_uuid',64)->comment('所属路局UUID');
            $table->index('belong_to_organization_railway_uuid');
            $table->string('belong_to_organization_paragraph_uuid',64)->comment('所属站段UUID');
            $table->index('belong_to_organization_paragraph_uuid');
            $table->string('belong_to_organization_workshop_uuid',64)->comment('所属车间UUID');
            $table->index('belong_to_organization_workshop_uuid');
            $table->string('belong_to_organization_work_area_uuid',64)->comment('所属工区UUID');
            $table->index('belong_to_organization_work_area_uuid');
            $table->string('use_place_current_organization_workshop_uuid',64)->comment('上道地点：车间UUID');
            $table->index('use_place_current_organization_workshop_uuid','entire_instances__upcowu');
            $table->string('use_place_current_organization_work_area_uuid',64)->comment('上道地点：工区UUID');
            $table->index('use_place_current_organization_work_area_uuid','entire_instances__upcowau');
            $table->string('use_place_current_location_line_uuid',64)->comment('上道地点：线别UUID');
            $table->index('use_place_current_location_line_uuid','entire_instances__upcllu');
            $table->string('use_place_current_location_station_uuid',64)->comment('上道地点：站场UUID');
            $table->index('use_place_current_location_station_uuid','entire_instances__upclsu');
            $table->string('use_place_current_location_section_uuid',64)->comment('上道地点：工区UUID');
            $table->index('use_place_current_location_section_uuid','entire_instances__upclsu2');
            $table->string('use_place_current_location_center_uuid',64)->comment('上道地点：中心UUID');
            $table->index('use_place_current_location_center_uuid','entire_instances__upclcu');
            $table->string('use_place_current_location_railroad_grade_cross_uuid',64)->comment('上道地点：道口UUID');
            $table->index('use_place_current_location_railroad_grade_cross_uuid','entire_instances__upclrgcu');
            $table->string('use_place_last_organization_workshop_uuid',64)->comment('上次上道地点：车间UUID');
            $table->index('use_place_last_organization_workshop_uuid','entire_instances__uplowu');
            $table->string('use_place_last_organization_work_area_uuid',64)->comment('上次上道地点：工区UUID');
            $table->index('use_place_last_organization_work_area_uuid','entire_instances__uplowau');
            $table->string('use_place_last_location_line_uuid',64)->comment('上次上道地点：线别UUID');
            $table->index('use_place_last_location_line_uuid','entire_instances__uplllu');
            $table->string('use_place_last_location_station_uuid',64)->comment('上次上道地点：站场UUID');
            $table->index('use_place_last_location_station_uuid','entire_instances__upllsu');
            $table->string('use_place_last_location_section_uuid',64)->comment('上次上道地点：工区UUID');
            $table->index('use_place_last_location_section_uuid','entire_instances__upllsu2');
            $table->string('use_place_last_location_center_uuid',64)->comment('上次上道地点：中心UUID');
            $table->index('use_place_last_location_center_uuid','entire_instances__upllcu');
            $table->string('use_place_last_location_railroad_grade_cross_uuid',64)->comment('上次上道地点：道口UUID');
            $table->index('use_place_last_location_railroad_grade_cross_uuid','entire_instances__upllrgcu');
            $table->string('use_place_current_position_indoor_cell_uuid',64)->comment('上道位置：室内上道位置UUID');
            $table->index('use_place_current_position_indoor_cell_uuid','entire_instances__upcpicu');
            $table->string('use_place_last_position_indoor_cell_uuid',64)->comment('上次上道位置：室内上道位置UUID');
