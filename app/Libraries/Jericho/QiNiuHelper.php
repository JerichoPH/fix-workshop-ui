...
class QiNiuHelper
{
    ...
    {
        $save_name = md5(TextHelper::rand() . strval(time() * 1000)) . '.' . $uploaded_file->extension();
        ...
        if (!$ret) return HttpResponseHelper::fail()->details($error);
        return HttpResponseHelper::success()->data([
            'filename' => $save_name,
    ...
    {
        $save_name = md5(TextHelper::rand() . strval(time() * 1000)) . '.' . pathinfo($file['name'], PATHINFO_EXTENSION);
