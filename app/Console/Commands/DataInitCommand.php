...
class DataInitCommand extends Command
{
	...
	private function _createRbac()
	{
		// 创建角色
		$rbacRole = RbacRole::with([])->create(['uuid' => Str::uuid(), 'name' => '默认']);
		$this->line("创建角色：$rbacRole->name");
		...
		// 创建角色→用户
		DB::table('pivot_rbac_role_and_accounts')->insert(['rbac_role_uuid' => $rbacRole->uuid, 'account_uuid' => $accountAdmin->uuid]);
		$this->line("角色绑定用户：$accountAdmin->nickname → $rbacRole->id");
		...
		collect([
			...
			->each(function ($rbacPermissionGroupUri, $rbacPermissionGroupName) use ($rbacRole) {
				...
				$rbacPermissionGroup = $this->_createPermissionGroup(
					...
					collect($subs)->each(function ($datum, $name) use ($rbacPermissionGroup, $rbacRole) {
						$rbacPermission = RbacPermission::with([])
							->create([
								...
								'sort' => 0,
								'name' => $name,
								'uri' => TextFacade::joinWithNotEmpty('/', [$this->option('prefix_url'), $datum['uri']]),
								'method' => $datum['method'],
						...
						$this->line("绑定角色与权限：{$rbacRole->name}→{$rbacPermission->name}");
					});
				}
			});
		...
		collect([
			...
			->each(function ($menu1) use ($rbacRole) {
				...
				collect($menu1['subs'])->each(function ($menu2) use ($newMenu1, $rbacRole) {
					...
					$this->line("创建菜单：$newMenu1->name → $newMenu2->name");
					DB::table('pivot_rbac_role_and_menus')->insert(['menu_uuid' => $newMenu2->uuid, 'rbac_role_uuid' => $rbacRole->uuid]);
					$this->line("角色绑定菜单：$rbacRole->name → $newMenu1->name($newMenu2->name)");
				});
			});
	}
	
	private function _createPositionDepotRowType()
	{
		// 创建仓库排类型
		collect([
			['unique_code' => 'FIXED', 'name' => '成品'],
			['unique_code' => 'EMERGENCY', 'name' => '应急备品'],
			['unique_code' => 'FIXING', 'name' => '待修']
		])
			->each(function ($datum) {
				...
				$this->line("创建仓库排：{$datum['name']}");
			});
	}
	
	public function _createEntireInstanceStatus()
	{
		// 创建器材状态
		collect([
			[
			...
			->each(function ($status) {
				...
				$this->line("创建器材状态：{$status['unique_code']} {$status['number_code']} {$status['name']}");
			});
	}
	
	private function _createEntireInstanceLogType()
	{
		// 创建器材日志类型
		collect([[
			'unique_code' => 'TAG',
			...
			->each(function ($type) {
				...
				$this->line("创建器材日志类型：{$type['unique_code']} {$type['name']}");
			});
	}
	...
	private function _createOrganization()
	{
		...
		$this->line("创建工区专业：{$power_supply_panel_profession->name}");
		...
		collect([
			...
			->each(function ($datum) use (
				...
				$this->line("创建路局：{$organization_railway->name}");
				...
				if ($organization_railway->unique_code == 'A12') {
					...
					collect([
						...
						->each(function ($datum) use (
							...
							$this->line("创建站段：{$organization_paragraph->name}");
							...
							$this->line("创建车间：{$fix_organization_workshop->name}");
							...
							$this->line("创建工区：{$point_switch_organization_work_area->name}");
							...
							$this->line("创建工区：{$relay_organization_work_area->name}");
							...
							$this->line("创建工区：{$synthesize_organization_work_area->name}");
							...
							$this->line("创建工区：{$power_supply_panel_organization_work_area->name}");
							...
							$this->line("创建车间：{$ele_organization_workshop->name}");
							...
							$this->line("创建车间：{$veh_organization_workshop->name}");
							...
							$this->line("创建车间：{$hump_organization_workshop->name}");
						});
				}
			});
	}
	 ...
	 */
	private function _createPermissionGroup(string $rbacPermissionGroupName, string $rbacPermissionGroupUri, RbacRole $rbacRole): RbacPermissionGroup
	{
		$rbacPermissionGroup = RbacPermissionGroup::with([])->create(['uuid' => Str::uuid(), 'name' => $rbacPermissionGroupName]);
		$this->line("创建权限分组：$rbacPermissionGroup->name");
			...
			use ($rbacPermissionGroupUri, $rbacPermissionGroup, $rbacRole) {
				$rbacPermission = RbacPermission::with([])
					->create([
						...
						'uri' => TextFacade::joinWithNotEmpty('/', [$this->option('prefix_url'), $rbacPermissionGroupUri, $datum['uri']]),
						'method' => $datum['method'],
						'rbac_permission_group_uuid' => $rbacPermissionGroup->uuid,
					]);
				$this->line("创建权限：$rbacPermissionGroup->name → $rbacPermission->name");
				// 创建角色→权限
				...
				$this->line("角色绑定权限：$rbacRole->name → $rbacPermissionGroup->name($rbacPermission->name)");
			});
	...
	private function testUser()
	{
		...
		$this->line("创建用户：$accountRailway->nickname");
		...
		$this->line("创建用户：$accountParagraph->nickname");
		...
		$this->line("创建用户：$accountWorkshop->nickname");
		...
		$this->line("创建用户：$accountPointSwitch->nickname");
		...
		$this->line("创建用户：$accountRelay->nickname");
		...
		$this->line("创建用户：$accountSynthesize->nickname");
		...
		$this->line("创建用户：$accountPowerSupplyPanel->nickname");
	...
	{
		$this->comment('初始化数据开始');
		$this->_truncateTables();  // 截断现有数据
		$this->_createRbac();  // 创建角色、权限、用户、菜单
		$this->_createPositionDepotRowType();  // 创建仓库排类型
		$this->_createEntireInstanceStatus();  // 创建器材状态
		$this->_createEntireInstanceLogType(); // 创建器材日志类型
		$this->_createOrganization();  // 创建归属单位
		$this->comment('初始化数据完成');
	}
