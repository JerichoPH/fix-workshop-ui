...
class CreateOrganizationStationsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('organization_stations', function (Blueprint $table) {
            ...
            $table->foreign("organization_workshop_unique_code","oStations__owuc")->references("unique_code")->on("organization_workshops")->onUpdate("cascade")->comment("所属车间");
            $table->char("organization_work_area_unique_code")->nullable(true)->comment("所属工区代码");
            $table->foreign("organization_work_area_unique_code","oStations__owauc")->references("unique_code")->on("organization_work_areas")->onUpdate("cascade")->comment("所属工区");
        });
    }
