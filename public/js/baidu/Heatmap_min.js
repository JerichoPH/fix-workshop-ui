(function(b, c, a) {
	...
	var h = (function j() {
		var p = function(q) {
			...
			return u.getImageData(0, 0, 256, 1)
				.data
		};
		var o = function(s, t) {
			var v = document.createElement("canvas");
...
(function() {
	...
	a.prototype.initialize = function(f) {
		...
		c.style.left = 0;
		c.style.border = 0;
		c.style.width = this._map.getSize()
			.width + "px";
		c.style.height = this._map.getSize()
			.height + "px";
		this.conf.element = c;
		if (!b()) {
			return c
	...
	a.prototype.draw = function() {
		...
		if (this.latlngs.length > 0) {
			...
			d = {
				max: this.heatmap._store.getData()
					.max,
				data: []
			};
			while (n--) {
				...
				var g = this._map.pointToOverlayPixel(f),
					i = this._map.pointToOverlayPixel(k.getSouthWest())
					.x,
					c = this._map.pointToOverlayPixel(k.getNorthEast())
					.y,
	...
	a.prototype.setDataSet = function(j) {
		...
		while (k--) {
			...
			var g = this._map.pointToOverlayPixel(f),
				h = this._map.pointToOverlayPixel(i.getSouthWest())
				.x,
				c = this._map.pointToOverlayPixel(i.getNorthEast())
				.y,
